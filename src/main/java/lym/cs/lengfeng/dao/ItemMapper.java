...
public interface ItemMapper {
    ...
    @Update("update items set name = #{item.name}, type = #{item.type}, description = #{item.description} where id = #{item.id}")
    ...
    @Insert("insert into items(name, type, description, owner) values(#{item.name}, #{item.type}, #{item.description}, #{item.owner})")
    @Options(useGeneratedKeys = true, keyProperty = "item.id")
