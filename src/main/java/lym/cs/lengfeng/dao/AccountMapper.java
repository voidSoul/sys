...
public interface AccountMapper {
    ...
    @Select("select user.truename, items.name, items.detail, items.type, account.money, account.description, account.date from  account " +
    ...
    @Select("select * from account where item_id = #{id} and owner = #{uid} or (select role from user where user.id = #{uid}) = 1")
